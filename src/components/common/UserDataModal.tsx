    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-[860px] h-[640px] bg-white dark:bg-[#1e1e1e] rounded-xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="h-12 border-b border-[var(--border)] px-5 flex items-center justify-between shrink-0 bg-white">
          <h2 className="text-[16px] font-bold text-[#1e1e1e]">Quản lý dữ liệu My Documents</h2>
          <button 
            onClick={onClose}
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-[230px] border-r border-[var(--border)] bg-[#f0f2f5] p-5 flex flex-col gap-6 shrink-0">
            <div>
              <div className="flex items-center justify-between mb-2">
                <span className="text-[13px] font-bold text-[#1e1e1e]">Dung lượng</span>
              ...
              <div className="mt-3 flex flex-wrap gap-x-3 gap-y-1.5">
                <div className="flex items-center gap-1.5 text-[11px] text-gray-600">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#fa8c16]" /> Ảnh
                </div>
                <div className="flex items-center gap-1.5 text-[11px] text-gray-600">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#52c41a]" /> Video
                </div>
                <div className="flex items-center gap-1.5 text-[11px] text-gray-600">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#ffc53d]" /> File
                </div>
                <div className="flex items-center gap-1.5 text-[11px] text-gray-600">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#eb2f96]" /> Voice
                </div>
                <div className="flex items-center gap-1.5 text-[11px] text-gray-600">
                  <span className="w-1.5 h-1.5 rounded-full bg-gray-400" /> Khác
                </div>
              </div>
            </div>
            ...
            <div className="mt-auto p-4 bg-[#e6f7ff] rounded-lg relative overflow-hidden border border-[#91d5ff]">
              <div className="relative z-10 flex flex-col items-center text-center gap-3">
                ...
                <div>
                  <h4 className="font-bold text-[13px] text-[#0068ff] mb-0.5">Nâng cấp dung lượng</h4>
                  <p className="text-[11px] text-[#0068ff]/80 leading-snug">Không gian lưu trữ lên đến 100 GB. Tự động bảo mật.</p>
                </div>
          ...
          <div className="flex-1 flex flex-col bg-white overflow-hidden">
            ...
            <div className="px-5 border-b border-[var(--border)] flex items-center gap-6 shrink-0">
               {[
                 { id: 'all', label: `Tất cả (${stats?.totalSizeFormatted || '0 B'})` },
                 { id: 'image', label: `Ảnh (${stats?.imageSizeFormatted || '0 B'})` },
                 { id: 'video', label: `Video (${stats?.videoSizeFormatted || '0 B'})` },
                 { id: 'file', label: `File (${stats?.fileSizeFormatted || '0 B'})` },
                 { id: 'voice', label: `Tin nhắn thoại (${stats?.voiceSizeFormatted || '0 B'})` },
               ].map(tab => (
                 <button 
                  key={tab.id}
