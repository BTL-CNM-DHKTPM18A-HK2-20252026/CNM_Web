...
export function ProfileModal({ isOpen, onClose, onUpdate, targetUserId, onStartChat, onAddFriend }: ProfileModalProps) {
  ...
  const validateAndUploadCover = async (file: File) => {
    ...
    try {
      ...
      const presignedRes = await apiClient.get<unknown>(
        `/users/me/presigned-url?fileName=${encodeURIComponent(file.name)}&fileType=${encodeURIComponent(file.type)}`
      );
      const presignedUrl =
        typeof presignedRes === 'string'
          ? presignedRes
          : (typeof presignedRes === 'object' && presignedRes !== null && 'data' in presignedRes && typeof (presignedRes as { data?: unknown }).data === 'string')
            ? (presignedRes as { data: string }).data
            : '';
